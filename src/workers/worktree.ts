 ...
 * changes the worker left uncommitted, then remove the worktree. The branch
 * stays for the record; the status gains `merged: true`.
 */
export function mergeWorker(logDir: string, id: string): string {
  ...
  saveStatus(logDir, s);
  const base = `merged ${st.branch} into ${st.cwd} (worktree removed)`;
