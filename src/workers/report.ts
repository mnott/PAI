...
function fullContractPrompt(format: "ag2" | "json"): string {
  const lines = [
    ...
    "Never sleep-poll: no `sleep N` over 60 seconds, standalone or leading a compound command.",
    "Long builds and tests run in background with no arbitrary `timeout` — the task notification",
    "wakes you; read the output then. Wait on a bounded timeout or event instead.",
    ...fileNotInlineBlock(),
    "",
