/**
 * planMerge/applyMerge — fold one registered project into another without
 * losing anything, against a raw better-sqlite3 Database.
 *
 * Moved out of src/registry/merge.ts (which keeps only the pure MergePlan
 * type and MergeError class) so that the one place in the codebase touching
 * better-sqlite3 for this operation is under src/storage/. Wrapped by
 * SQLiteRegistryBackend.planProjectMerge/applyProjectMerge
 * (../registry-sqlite.ts).
 *
...
import { MergeError, type MergePlan } from "../../registry/merge.js";
