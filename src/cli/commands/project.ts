...
import { existsSync, readdirSync, readFileSync, writeFileSync, statSync, mkdirSync, renameSync } from "node:fs";
...
// ---------------------------------------------------------------------------
// Name / Unname / Names — curated project shortlist via aliases
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Session config — per-project launch configuration
// ---------------------------------------------------------------------------
...
const PERMISSION_PRESETS: Record<string, Partial<SessionConfig>> = {
  full: {
    permission: 'full',
...
function getSessionConfig(project: ProjectRow): SessionConfig {
  return project.session_config ? JSON.parse(project.session_config) : {};
}

function getGlobalDefaults(): SessionConfig {
  try {
    const configPath = join(_homedir(), '.config', 'pai', 'config.json');
...
function saveGlobalDefaults(defaults: SessionConfig): void {
  const configPath = join(_homedir(), '.config', 'pai', 'config.json');
  let config: any = {};
  try {
    if (existsSync(configPath)) {
  ...
  mkdirSync(join(_homedir(), '.config', 'pai'), { recursive: true });
...
function cmdName(
  ...
  // Validate shortname: alphanumeric, hyphens, underscores only
  ...
  if (!conflictAlias) {
    db.prepare(
      "INSERT INTO aliases (alias, project_id) VALUES (?, ?)"
    ).run(shortname, project.id);
  }

  // Update session_config if permission specified
  if (opts.permission) {
    const existing = getSessionConfig(project);
    const config = applyPreset(existing, opts.permission);
    db.prepare(
      "UPDATE projects SET session_config = ?, updated_at = ? WHERE id = ?"
...
function cmdUnname(db: Database, shortname: string): void {
  const alias = db
    .prepare("SELECT project_id FROM aliases WHERE alias = ?")
    .get(shortname) as { project_id: number } | undefined;
  ...
  // Check if project has any remaining aliases
  const remaining = db
...
function cmdNames(db: Database, opts: { json?: boolean }): void {
  const rows = db.prepare(`
  ...
  if (opts.json) {
    // Group aliases per project for clean JSON
    ...
    for (const row of rows) {
      if (!grouped.has(row.id)) {
        ...
        grouped.get(row.id).names.push(row.name);
      }
    }
  ...
  // Deduplicate: group aliases per project
  ...
  console.log(
    renderTable(
      ["Name(s)", "Slug", "Path", "Permission", "Sessions", "Last Active"],
      tableRows
    )
  );
  console.log();
...
// ---------------------------------------------------------------------------
// Config — CRUD for session launch configuration
// ---------------------------------------------------------------------------
...
function cmdConfig(
  ...
  if (opts.defaults) {
    ...
    if (opts.set?.length) {
      for (const pair of opts.set) {
        ...
        if (key === 'env') {
          defaults.env = { ...(defaults.env ?? {}), ...parseConfigValue('env', value) };
        } else {
          (defaults as any)[key] = parseConfigValue(key, value);
        }
      }
      saveGlobalDefaults(defaults);
  ...
  // Per-project config — identifier required
  if (!identifier) {
  ...
  if (opts.reset) {
    db.prepare(
      "UPDATE projects SET session_config = NULL, updated_at = ? WHERE id = ?"
    ).run(now(), project.id);
  ...
  if (opts.set?.length) {
    for (const pair of opts.set) {
      ...
      if (key === 'env') {
        config.env = { ...(config.env ?? {}), ...parseConfigValue('env', value) };
      } else {
        (config as any)[key] = parseConfigValue(key, value);
      }
    }
    db.prepare(
      "UPDATE projects SET session_config = ?, updated_at = ? WHERE id = ?"
  ...
  // Show current config (always, after any mutations)
...
export function registerProjectCommands(
  ...
  projectCmd
    ...
    .description("Give a project a short name for quick access (used by AIBroker to launch sessions)")
    .option("--permission <level>", "Permission level: full | trusted | default (or raw CLI flags)")
    .action((identifier: string, shortname: string, opts: { permission?: string }) => {
  ...
  projectCmd
    ...
    .option("--set <key=value...>", "Set config values (repeatable)", (v: string, prev: string[]) => [...prev, v], [] as string[])
    .option("--unset <key...>", "Remove config keys (repeatable, use env.KEY for env vars)", (v: string, prev: string[]) => [...prev, v], [] as string[])
    ...
    .action((identifier: string | undefined, opts: {
      set?: string[];
      unset?: string[];
      preset?: string;
      defaults?: boolean;
